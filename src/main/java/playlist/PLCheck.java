...
public class PLCheck {
    ...
    private static void openFile() {
        try {
            ...
            JOptionPane.showMessageDialog(null, "              Чё нада?!", "Опа!", JOptionPane.ERROR_MESSAGE);
        }  // "Файл не найден"
    }
    ...
    static void checkFormats(Event[] events) {
        ...
        for (event = 1; event < totalEvents; event++) {
            ...
            for (int i = 0; i < tempFormat.length; i++) {
                ...
                if (tempFormat[i].contains("logo") || tempFormat[i].contains("Logo"))
                    tempLogo++;
            }
            ...
            if ((tempLogo) != 0) {
                errLogoFormat++;
                errors[event][2] = "   Warning! Are you sure here should be a logo?";
            }
...
//                }
        ...
        for (event = 1; event < totalEvents; event++) {
            if (!isThisProgramsSubclip(event) && !(events[event].getName().contains("MS-Fixies-Aeroplan") || events[event].getName().contains("T-Tonometr"))) {
                tempFormat = events[event].getFormat();
                // Если это траурный день, то может быть 3 формата (PGM, Svecha, Logo Traur)
                ...
                if (tempLogoSvecha == 0) {
                    if (logoV2 == 0) {
                        if (tempFormat.length != 1) {
                            errors[event][12] = "   format_ERROR!_(must be 1 formats - [PGM] )";
                            errFormat++;
                        }
                    } else {
                        if (tempFormat.length != 2) {
                            errors[event][12] = "   format_ERROR!_(must be 2 formats - [PGM, logo v2] )";
                            errFormat++;
                        }
                    }
                    ...
                    if (logoTraur2 == 0) {
                        if (tempFormat.length != 2) {
                            errors[event][12] = "   format_ERROR!_(must be 2 formats - [PGM, Svecha] )";
                            errFormat++;
                        }
                    } else {
                        if (tempFormat.length != 3) {
                            errors[event][12] = "   format_ERROR!_(must be 3 formats - [PGM, Svecha, logo Traur] )";
                            errFormat++;
                        }
                    }
                }
...
/*
    ...
    static void statistic(Event[] events) {
        ...
        if (errLogoFormat != 0) System.out.println("Уведомлений в format  logo: " + errLogoFormat);
